...
class WeiboOp:
    ...
    def like_blog(self, blog_mid):
        ...
        try:
            res = json.loads(res)
            if res['code'] == '100000':
                logging.warning("%s like_blog %s success!" % (self.uid, blog_mid))
                return True
            else:
                logging.warning("%s like_blog %s fail... msg: %s" % (self.uid, blog_mid, res['msg']))
                return False
        except:
            logging.warning("%s like_blog %s fail... msg: %s" % (self.uid, blog_mid, res))
            return False
    ...
    # 评论和转发
    def comment_forward(self, blog_mid, content, forward=False, img_url=None, img_id=None): # 目标微博id, 评论者的uid，评论内容，forward = {"0", "1"}是否转发
        payload = {
            'act': 'post', 
            'mid': blog_mid,
            'uid': self.uid,
        ...
        try:
            ...
            if res['code'] == '100000':
                logging.warning("comment_forward success! comments: %s" % content)
            else:
                logging.warning("%s comment_forward %s fail... code: %s msg: %s" % (self.uid, blog_mid, res['code'], res['msg']))
    ...
    # reply a comment
    def reply_comment(self, mid, comment_id, content, approval=False
        ...
        if approval is False:
            payload["approvalComment"] = "false"
        else:
            payload["approvalComment"] = "true"
        ...
        try:
            ...
            if res['code'] == '100000':
                ...
                logging.warning(
                    "%s comment_forward %s fail... code: %s msg: %s" % (self.uid, comment_id, res['code'], res['msg']))
        except Exception as e:
            print("返回信息非json")
            logging.warning(e)
